...
def jishi():
    ...
    plt.ylim(0,300)
    plt.xticks(x,all_class)
    plt.bar(x,data['rule'],bottom=data['brain'],label='违纪')
    plt.bar(x,data['shadow'],bottom=data['brain'],label='投影')
    plt.bar(x,data['sill'],bottom=data['brain'],label='未关窗')
