...
for i in range(len(skill_type)):
    if company_location[i*2+1][1].split('<em class="vline"></em>') == 1:
        a = (company_location[i*2][0].split(' ')[0],skill_type[i],company_name[i],company_location[i*2+1][1].split('<em class="vline"></em>')[0],company_location[i*2+1][0],
             company_info[i][1].replace('<br/>','').replace('&mdash;','').replace('&ldquo;','').replace('&rdquo;','').replace('&nbsp;',' ').replace(' ',''),company_location[i*2][0],
             company_info[i][0].replace(' ','').replace('<br/>',''),
             company_location[i*2][1].split('<em class="vline"></em>')[1],
             company_location[i*2][1].split('<em class="vline"></em>')[0],
             company_welfare[i])
    else:
        a = (company_location[i*2][0].split(' ')[0],skill_type[i],company_name[i],company_location[i*2+1][1].split('<em class="vline"></em>')[1],company_location[i*2+1][0],
             company_info[i][1].replace('<br/>','').replace('&mdash;','').replace('&ldquo;','').replace('&rdquo;','').replace('&nbsp;',' ').replace(' ',''), company_location[i * 2][0],
             company_info[i][0].replace(' ','').replace('<br/>',''),
             company_location[i * 2][1].split('<em class="vline"></em>')[1],
             company_location[i * 2][1].split('<em class="vline"></em>')[0],
...
df.to_csv('爬虫.csv',header = True,mode='a', encoding="utf_8_sig",index=False)
