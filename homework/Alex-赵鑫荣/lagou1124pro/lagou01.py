...
class lagouzp:
    ...
    def to_datacsv(self,data,
                     columns = ['公司全名','公司简称','公司规模','融资阶段','区域',
                                '职位名称','工作经验','学历要求','工资','职位福利']):
        df = pd.DataFrame(data=data, columns=columns)
        #print(df)
        try:
            pd.read_csv('lagou{}.csv'.format(self.kd))
            print(1)
        except:
            print(2)
            df.to_csv('lagou{}.csv'.format(self.kd), index=False, encoding='utf-8-sig')
        else:
            df.to_csv('lagou{}.csv'.format(self.kd), mode='a', index=False, header=False, encoding='utf-8-sig')
        print('保存完成！')
        return '保存完成！'
